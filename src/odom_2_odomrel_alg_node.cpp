...
algorithm_base::IriBaseAlgorithm<Odom2OdomrelAlgorithm>()
{
  ...
  last_pose_(1) = 0;
  last_pose_(2) = 0;
  public_node_handle_.param<double>("covariance_factor", covariance_factor_, 0.1);
...
void Odom2OdomrelAlgNode::odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  for (uint i = 0; i < 3; i++) 
    cov(i, i) = std::max(covariance_factor_ * covariance_factor_ * odomrel(i) * odomrel(i), 1e-6);
  ...
  new_odomrel.twist.twist.angular.x = last_odom_.twist.twist.angular.x - msg->twist.twist.angular.x;
  new_odomrel.twist.twist.angular.y = last_odom_.twist.twist.angular.y - msg->twist.twist.angular.y;
  new_odomrel.twist.twist.angular.z = last_odom_.twist.twist.angular.z - msg->twist.twist.angular.z;

  new_odomrel.twist.twist.linear.x = last_odom_.twist.twist.linear.x - msg->twist.twist.linear.x;
  new_odomrel.twist.twist.linear.y = last_odom_.twist.twist.linear.y - msg->twist.twist.linear.y;
  new_odomrel.twist.twist.linear.z = last_odom_.twist.twist.linear.z - msg->twist.twist.linear.z;
